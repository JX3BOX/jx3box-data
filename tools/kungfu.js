...
function buildKungfuMap(client, filename) {
    // 读取心法与套路关系
    let data = fs.readFileSync(`./raw/bps/${client}/skill_mkungfu_kungfu.txt`);
    ...
    // 读取套路与技能关系
    let _data = fs.readFileSync(`./raw/bps/${client}/skill_kungfu.txt`, "utf-8");
    ...
    fs.writeFileSync(`./data/bps/${filename}`, JSON.stringify(kungfumap));
}
